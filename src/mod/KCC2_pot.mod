...
NEURON {
	...
	USEION k READ ik, ko, ek WRITE ki VALENCE 1
	RANGE cli0, diff
	RANGE efflux, Pa
	RANGE itonic, gtonic
	GLOBAL celsius
...
KINETIC scheme {
	...
	: synaptic chloride current (circumference) - KCC2 efflux (circumference)
	~ cli << ((1e4)*icl*diam*PI/F - (1e4)*efflux*diam*PI/F + - (1e4)*itonic*diam*PI/F)
	~ ki << ((1e4)*efflux*diam*PI/F - (1e4)*itonic*diam*PI/F)
}  
